...
stage_two_model = PeftModel.from_pretrained(base_model2, stage_two_model_save_name)
stage_two_model = stage_two_model.merge_and_unload()
stage_two_tokenizer = AutoTokenizer.from_pretrained(stage_two_model_save_name, trust_remote_code=True)

stage_two_model.pad_token_id = stage_two_tokenizer.pad_token_id
stage_two_model.config.pad_token_id = stage_two_tokenizer.pad_token_id
stage_two_pipe = pipeline(task="text-generation", model=stage_two_model, tokenizer=stage_two_tokenizer, max_new_tokens=1, return_full_text=False)



@api.route('/classify_intent_round_two', methods=['POST'])
def intent_stage_two():
    ...
    try:
        for req in data:
            uid = req['uid']
            params = req['params']
            app_log.info('**** Stage Two Intent Api')
            app_log.info('Input Prompt: {}'.format(params['prompt']))
            outputs = stage_two_pipe(params['prompt'])
            sub_res = {'uid': uid,
    ...
    output = {'result':results, 'success':success, 'error_msg': 'Stage Two Intent Api: {}'.format(error_msg)}
...
def report_self_alive(redis_obj, server_addr, dev):
    try:
        ...
        core_key_intent_rb = "{aigc_imgintent_rb}"

        core_keys = [core_key_intent_rb]
